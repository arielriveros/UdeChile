...
int nMain(int argc, char **argv)
{
  ...
  ball_gen= nEmitTask(BallGen, balls, Real2LL(vel));

  climber= nEmitTask(Climber, 1, 24, Real2LL(vel));
  nWaitTask(climber);
...
long long llvel;
{
  ...
  while ((req= (Request *) nReceive( &sender, wakeup ))==NULL)
  {
    ...
    balls[curr_ball]= nEmitTask(Ball, 1, 1, Real2LL(rand_vel));
...
long long llvel;
{
  ...
  while ( ! quit_game )
  {
    ...
    { nTask sender;
      ...
      (void)nReceive(&sender, (int)pause);
      Restore(y, x);
  } }
